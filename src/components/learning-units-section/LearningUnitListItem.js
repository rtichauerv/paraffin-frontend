...
function LearningUnitItem({ unit, showSuccess }) {
  ...
  const putCompletedLearningUnitEndpoint = endpoints('putLearningUnitCompleted', unit.id);

  const { data: isCompleted, isLoading, isError, mutate } = useGet(completedLearningUnitEndpoint);

  const changeHandler = (clicked) => {
    const requestOptions = {
      method: clicked.value ? 'PUT' : 'DELETE',
      headers: { 'Content-Type': 'application/json' },
    };
    fetch(putCompletedLearningUnitEndpoint, requestOptions).then((response) => {
      if (response.ok) {
        mutate();
        showSuccess();
      }
